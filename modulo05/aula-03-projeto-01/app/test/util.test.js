...
describe('Util', () => {
  it('#evaluateRegex should throw an error using an unsafe regex', () => {
    ...
    // time \
    // node --eval "/^([a-z|A-Z|0-9]+\s?)+$/.test('eaaaae man como vai voce e como vai voce e como vai voce?') && console.log('legalzin')"

    expect(() => evaluateRegex(unsafeRegex)).to.throw(InvalidRegexError, `Invalid regex expression: ${unsafeRegex}`)
  });
